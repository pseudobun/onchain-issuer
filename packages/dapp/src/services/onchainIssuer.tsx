...
import type { Id } from '@iden3/js-iden3-core';
import { balanceIssuerAbi } from '../abi/balanceCredentialIssuer';
...
export const userCredentialIds = async (
  contractAddress: string,
  userId: Id
...
export const getCredential = async (
  ...
  const web3 = new Web3(window.ethereum);
  const functionAbi = balanceIssuerAbi.find(
...
export const getOnchainIssuerVersion = async (
  contractAddress: string
): Promise<string> => {
  const web3 = new Web3(window.ethereum);
  const contract = new web3.eth.Contract(balanceIssuerAbi, contractAddress);
